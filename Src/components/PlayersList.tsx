...
export function PlayersList({ onViewPlayer, onEditPlayer, onCreatePlayer }: PlayersListProps) {
  ...
  const handleDelete = async (playerId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault(); // Seguridad extra para móviles
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-orange-500 font-black text-[10px] uppercase tracking-widest">Official Circuit</span>
          </div>
          ...
          {filteredPlayers.map((player, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                {/* CORRECCIÓN: Botones con stopPropagation y z-index alto */}
                {canModify(player) && (
                  <div className="flex flex-col gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity z-20">
                    <button 
                      onClick={(e) => { 
                        e.stopPropagation(); 
                        e.preventDefault(); 
                      ...
                      className="p-2 bg-gray-800 hover:text-orange-500 rounded-xl transition-all active:scale-90"
                    >
                      <Edit size={14} />
                    </button>
                    <button 
                      ...
                      className="p-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all active:scale-90"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
